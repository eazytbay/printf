...
for (format_index = 0; format_string && format_string[format_index]
!= '\0'; format_index++)
{
...
format_precision = fetch_custom_precision(format_string,
&format_index, arguments);
format_size = fetch_custom_size(format_string, &format_index);
format_index++;
printed_chars = handle_print_format(format_string, &format_index, arguments,
output_buffer, format_flags, format_width, format_precision, format_size);
if (printed_chars == -1)
return (-1);
